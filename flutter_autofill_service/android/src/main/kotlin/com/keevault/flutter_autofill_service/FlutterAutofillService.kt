...
class FlutterAutofillService : AutofillService() {
    ...
    override fun onFillRequest(
        ...
        //TODO:
        ///I MADE THIS FALSE SO THAT USERS ARE NOT PROMPTED TO SAVE A PASSWORD
        /// WHEN IT IS FILLED OUT ON THE WEB. I DO NOT WANT TO REMOVE THE BLOCK OF
        /// CODE NOW. WILL PROBABLY NEED TO REVIEW LATER

        val offerToSave = false
//        val offerToSave = autofillPreferenceStore.autofillPreferences.enableSaving
//        logger.debug { "Offer to save request: $offerToSave" }
...
//            logger.info { "Debug: No password fields detected ($detectedFields total). Non-manual request so aborting." }
        ...
        /// I need to determine the device manufacturer because a bug is noticed on
        /// Samsung devices running on android 14 dark mode. The text colors are a little bit
        /// invisible because it is the same with the background color.
        /// So we are statically setting the color only for samsung devices running on android 14
        /// The function to statically set the color can be found below.
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                if(manufacturer == "samsung"){
                    presentationMenu.setTextColor(R.id.text, Color.BLACK)
                }
            ...
            fillResponseBuilder.setAuthentication(
                autoFillIds.toTypedArray(), intentSender,
                RemoteViewsHelper.viewsWithAuth(packageName, useLabel, unlockDrawableId),
            );
        }
//        logger.info {
...
private fun MutableList<AutofillHeuristic>.autofillHint(
    weight: Int,
    hint: String,
    block: Boolean? = false
) =
...
private fun MutableList<AutofillHeuristic>.nonAutofillHint(
    weight: Int,
    matches: List<String>,
    block: Boolean? = false
) =
    heuristic(weight, "naHint", block) {
        hint?.lowercase()?.let { h -> matches.any { it in h } } ?: false
    }

private fun MutableList<AutofillHeuristic>.idEntryExact(
    weight: Int,
    match: String,
    block: Boolean? = false
) =
...
private fun MutableList<AutofillHeuristic>.idEntry(
    weight: Int,
    matches: List<String>,
    block: Boolean? = false
) =
    heuristic(weight, "id", block) {
        idEntry?.lowercase()?.let { i -> matches.any { it in i } } ?: false
    }
...
private fun MutableList<AutofillHeuristic>.contentDescription(
    weight: Int,
    matches: List<String>,
    block: Boolean? = false
) =
    heuristic(weight, "contentDescription", block) {
        contentDescription?.toString()?.lowercase()?.let { cd -> matches.contains(cd) } ?: false
    }
...
enum class AutofillInputType(val heuristics: List<AutofillHeuristic>) {
    Password(mutableListOf<AutofillHeuristic>().apply {
        ...
        heuristic(
            239,
            "text variation web password"
        ) { inputType.hasFlag(android.text.InputType.TYPE_TEXT_VARIATION_WEB_PASSWORD) }
        heuristic(
            238,
            "text variation visible password"
        ) { inputType.hasFlag(android.text.InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD) }
